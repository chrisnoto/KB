...
data "template_file" "user_data" {
  ...
  vars = {
    ...
    hostname = "victoriaM${count.index + 3}"
...
resource "null_resource" "cloud_init_user_data_files" {
  ...
  provisioner "file" {
    ...
    destination = "/var/lib/vz/snippets/victoriaM_user_data-${count.index}.yml"
  }
}

resource "proxmox_vm_qemu" "vic" {
  depends_on = [
    null_resource.cloud_init_user_data_files
  ]

  count        = var.vm_count
  name         = "victoriaM${count.index + 3}"
  target_node  = "pve1"
  pool         = "chensen"
  ...
  ipconfig0= "ip=10.67.50.5${count.index + 3}/23,gw=10.67.50.1"
  cicustom = "user=local:snippets/victoriaM_user_data-${count.index}.yml"
  ciuser   = "centos"
}

output "vm_ip_address" {
  description = "The IP addr of VM"
  value = proxmox_vm_qemu.vic.*.default_ipv4_address
}
...
resource "local_file" "inventory" {
  ...
  content = <<EOF
  [app]
  ${proxmox_vm_qemu.vic[0].default_ipv4_address}
  EOF
}
